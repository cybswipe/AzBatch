...
if ($renderer -like "vray*")
{
    ...
    $pre_render_script_content += "if (indexVrayGPU == 0) then (r.system_vrayLog_level = 4; r.system_vrayLog_file = ""$vrayLogFile"")`r`n"
...
{
        $max_exec = $env:ADSK_3DSMAX_x64_2022+ "3dsmaxcmd.exe"
        if(-Not (Test-Path "$env:ADSK_3DSMAX_x64_2022"))
        {
            Write-Host "3ds Max 2022 doesn't exist on this rendering image, please use a newer version of the rendering image."
            exit 1
        }
}
Else 
...
If ($maxVersion -eq "2022")
{  
    Copy-Item "${env:LOCALAPPDATA}\Autodesk\3dsMax\2022 - 64bit\ENU\Network\Max.log" .\Max_full.log -ErrorAction SilentlyContinue 
}
